...
const News = (props) => {
  ...
  //   },
  // ];
  // document.title = `${cat === "general" ? "Home" : this.heading} - NEWSApp`
  ...
  useEffect(() => {
    document.title = `${cat === "general" ? "Home" : heading} - NEWSApp`;
    updateNews();
  ...
  return (
    <>
      <h2 className="text-center  mt-5 py-3 pb-1">Top {heading} Headlines</h2>
      {loading && <Spinner />}
      ...
      {/* <div className="d-flex justify-content-between my-2">
          ...
          <button disabled={this.state.page + 1 > Math.ceil(this.state.totalResults/props.pageSize)} type="button" className="btn btn-dark" onClick={this.handleNextClick}> &rarr; Next </button>
        </div> */}
    </>
  );
};
